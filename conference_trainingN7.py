...
class SumoIntersection:
    ...
    def chooseMode(self, junctions):
        ...
        averageQueueLength = traci.edge.getLastStepHaltingNumber('R32') + traci.edge.getLastStepHaltingNumber(
                        'R30') + traci.edge.getLastStepHaltingNumber('R36') + traci.edge.getLastStepHaltingNumber('R34')
        averageQueueLength = averageQueueLength/4

        # Define vehicleId's Of Emergency Vehicles
        emergencyVIds = []
        vehicles_road1 = traci.edge.getLastStepVehicleIDs('R32')
        vehicles_road2 = traci.edge.getLastStepVehicleIDs('R30')
        vehicles_road3 = traci.edge.getLastStepVehicleIDs('R36')
        vehicles_road4 = traci.edge.getLastStepVehicleIDs('R34')
    ...
    def calculateReward(self, wgMatrix):
        ...
        laness=['R32_0','R30_0','R36_0','R34._0']
    ...
    def getState(self):
        ...
        junctionPosition = traci.junction.getPosition('N7')[0]
        vehicles_road1 = traci.edge.getLastStepVehicleIDs('R32')
        vehicles_road2 = traci.edge.getLastStepVehicleIDs('R30')
        vehicles_road3 = traci.edge.getLastStepVehicleIDs('R36')
        vehicles_road4 = traci.edge.getLastStepVehicleIDs('R34')
...
if __name__ == '__main__':
    ...
    for e in range(episodes):
        ...
        #PORT = PORT + 1
        sumoBinary = checkBinary('sumo-gui')   #changed here
        ...
        traci.trafficlight.setPhase("N7", 0)
        traci.trafficlight.setPhaseDuration("N7", 200)
        cpd1=traci.trafficlight.getPhaseDuration("N7")
        print("cycle phase duration initial", cpd1)
        cpd=cpd1
        turn = 0
        # contains vehicle to its wg matrix
        weight_matrix = []
        while traci.simulation.getMinExpectedNumber() > 0 and stepz < 1000:   #changed here
            ...
            if(action==0 or action == 1 or action ==2):
                ...
                for i in range(5):
                    stepz += 1
                    traci.trafficlight.setPhase("N7", turn)
                ...
                if (cpd>=20 and cpd<=80):
                    traci.trafficlight.setPhaseDuration("N7", cpd)
                else:
                    traci.trafficlight.setPhaseDuration("N7", 80)
            ...
            if(action==3 or action == 4 or action ==5):
                ...
                cpd=cpd1-(ch[action]*cpd1)/100
                if (cpd>=20 and cpd<=80):
                    traci.trafficlight.setPhaseDuration("N7", cpd1)
                else:
                    traci.trafficlight.setPhaseDuration("N7", 20)
