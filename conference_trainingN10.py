...
try:
    sys.path.append(os.path.join(  # tutorial in tests
...
class DQNAgent:
    ...
    def _build_model(self):
        ...
        input_1 = Input(shape=(3, 1))   #changed here
        ...
        input_2 = Input(shape=(3, 1))   #changed here
        ...
        model.compile(optimizer=tensorflow.keras.optimizers.RMSprop(lr=self.learning_rate),
                      loss='mse')
...
class SumoIntersection:
    ...
    def chooseMode(self, junctions):
        ...
        averageLoad = self.calculateReward(wgMatrix)[0] / 3
        averageQueueLength = traci.edge.getLastStepHaltingNumber('R91') \
            + traci.edge.getLastStepHaltingNumber('R31') \
            + traci.edge.getLastStepHaltingNumber('R50')
        averageQueueLength = averageQueueLength / 3

        # Define vehicleId's Of Emergency Vehicles

        emergencyVIds = []
        vehicles_road1 = traci.edge.getLastStepVehicleIDs('R91')
        vehicles_road2 = traci.edge.getLastStepVehicleIDs('R31')
        vehicles_road3 = traci.edge.getLastStepVehicleIDs('R50')
    ...
    def getState(self):
        ...
        junctionPosition = traci.junction.getPosition('N10')[0]
        vehicles_road1 = traci.edge.getLastStepVehicleIDs('R91')
        vehicles_road2 = traci.edge.getLastStepVehicleIDs('R31')
        vehicles_road3 = traci.edge.getLastStepVehicleIDs('R50')
        ...
        phase = traci.trafficlight.getPhaseDuration('N10')

        ql1 = np.reshape(ql, (1, 3, 1))

        wt1 = np.reshape(wt, (1, 3, 1))
...
if __name__ == '__main__':
    ...
    for e in range(episodes):
        ...
        traci.trafficlight.setPhase('N10', 0)
        traci.trafficlight.setPhaseDuration('N10', 200)
        cpd1 = traci.trafficlight.getPhaseDuration('N10')
        ...
        weight_matrix = []
        while traci.simulation.getMinExpectedNumber() > 0 and stepz \
            < 10000:
            ...
            if action == 0 or action == 1 or action == 2:
                 turn = (turn + 1) % 3  # Ensure turn is within the range [0, 2]
                 reward1 = sumoInt.calculateReward(weight_matrix)[0]
                 for i in range(5):
                      stepz += 1
                      traci.trafficlight.setPhase("N10", turn)
                 ...
                 if cpd >= 20 and cpd <= 80:
                    traci.trafficlight.setPhaseDuration('N10', cpd1)
                 else:
                    traci.trafficlight.setPhaseDuration('N10', 20)
            ...
            if action == 3 or action == 4 or action == 5:
                ...
                if cpd >= 20 and cpd <= 80:
                    ...
                    traci.trafficlight.setPhaseDuration('N10', 20)
                print ('cycle phase duration final and action, action-'
                       , cpd, action)
